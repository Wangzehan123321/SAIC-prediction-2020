...
from SAIC_utils import ArgoverseDataset,MSE,MSElastdistance
...
class Visualizer(object):
    ...
    def txt(self,name,text,**kwargs):
        ...
        self.index[name]=x+1
vis=Visualizer('argo_lstm_pretrain')
...
trSet = ArgoverseDataset('./pkl_file/argoverse_train.pkl',social_input=args["social_input"])#train datatset
valSet = ArgoverseDataset('./pkl_file/argoverse_val.pkl',social_input=args["social_input"])#val dataset
...
for epoch_num in range(trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        if args['use_maneuvers']:
            fut_pred_all, lon_pred = net(hist_traj_batch)
            # 采用最后一个位置的坐标与真值之间距离最小的仲裁方式确定属于哪一类
    ...
    with torch.no_grad():
        ...
        for j, data in enumerate(valDataloader):
            ...
            if args['use_maneuvers']:
                fut_pred_all, lon_pred = net(hist_traj_batch)
                # 采用最后一个位置坐标与真值之间距离最小的仲裁方式确定属于哪一类
                fut_1, fut_2, fut_3 = fut_pred_all
                fut_pre_pred = torch.zeros_like(fut_1)
                l_1 = MSElastdistance(fut_1, fut_batch)
                l_2 = MSElastdistance(fut_2, fut_batch)
    ...
    torch.save(net.state_dict(), './trained_models/argo_lstm_pretrain/pretrained_net_{}.tar'.format(str(epoch_num)))
